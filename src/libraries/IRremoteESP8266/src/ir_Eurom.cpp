...
void IREuromAc::setTemp(const uint8_t degrees, const bool fahrenheit) {
  ...
  if (fahrenheit) {
    temp_f = ::max(kEuromMinTempF, degrees);
    temp_f = ::min(kEuromMaxTempF, temp_f);
  ...
  temp_c = ::max(kEuromMinTempC, temp_c);
  temp_c = ::min(kEuromMaxTempC, temp_c);
...
void IREuromAc::setOffTimer(const uint8_t duration) {
  uint8_t hours = ::max(kEuromTimerMin, duration);
  hours = ::min(kEuromTimerMax, hours);
