...
void IRKelon168Ac::setRaw(const uint8_t newCode[], const uint16_t length) {
  std::memcpy(_.raw, newCode, ::min(length, kKelon168StateLength));
}
...
void IRKelon168Ac::_setTemp(const uint8_t temp, const bool remember) {
  ...
  uint8_t newtemp = ::max((uint8_t)(kKelon168MinTemp + offset), temp);
  newtemp = ::min((uint8_t)(kKelon168MaxTemp + offset), newtemp);
