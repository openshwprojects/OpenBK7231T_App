...
static void ResetEventsAndChannels(int eventsCleared) {
	SELFTEST_ASSERT(CLOCK_ClearEvents() == eventsCleared);
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 0);
...
void Test_ClockEvents() {
	...
	// now there is 1
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 1);
	// none removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(1245) == 0);
	// one removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(123) == 1);
	// now there is 0
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 0);
	...
	// now there is 2
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 2);
	CMD_ExecuteCommand("addClockEvent 15:30:00 0xff 1003 POWER0 ON", 0);
	// now there is 3
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 3);
	CMD_ExecuteCommand("addClockEvent 15:30:00 0xff 1004 POWER0 ON", 0);
	// now there is 4
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 4);
	// one removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(1004) == 1);
	// now there is 3
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 3);
	// one removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(1003) == 1);
	// now there is 2
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 2);
	...
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 4);
	// one removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(1005) == 1);
	// now there is 3
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 3);
	// one removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(1001) == 1);
	// now there is 2
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 2);
	// one removed
	SELFTEST_ASSERT(CLOCK_RemoveEvent(1002) == 1);
	// now there is 1
	SELFTEST_ASSERT(CLOCK_Print_EventList() == 1);
	...
	SELFTEST_ASSERT(CLOCK_GetEventTime(1006) == 15 * 3600 + 30 * 60);
	SELFTEST_ASSERT(CLOCK_GetEventTime(1007) == 8 * 3600 + 9 * 60);
	SELFTEST_ASSERT(CLOCK_GetEventTime(1008) == 9 * 3600 + 8 * 60 + 7);
	...
	for (int i = 0; i < 100; i++) {
		CLOCK_RunEvents(simTime + i, true);
	}
