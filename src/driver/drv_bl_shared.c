...
int dump_load_on = 10;		// The ammount of 'excess' energy stored over the period. Above this, the dump load will be turned on.
...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
	...
	if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE))
	{
		...
		if (check_time - lastsync >= dump_load_hysteresis) {
			...
			if ((min_production>(sensors[OBK_POWER].lastReading)&&(net_energy>dump_load_off))||(net_energy>dump_load_on))
			{
				dump_load_relay = 1;
