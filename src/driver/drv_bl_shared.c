...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
	...
	poststr(request, "<tr><td><b>Total Generation</b></td><td style='text-align: right;'>");
        hprintf255(request, "%.3f</td><td>KWh</td>", ([OBK_GENERATION_TOTAL].lastReading * 0.001); //always display OBK_GNERATION_TOTAL in kwh
...
void BL_ProcessUpdate(float voltage, float current, float power,
    ...
    sensors[OBK_GENERATION_TODAY].lastReading += (double)generation;
    energyCounterStamp = xTaskGetTickCount();
