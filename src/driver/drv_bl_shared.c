...
void BL09XX_AppendInformationToHTTPIndexPage(http_request_t *request)
{
	...
	{	
		...
		if (CFG_HasFlag(OBK_FLAG_POWER_ALLOW_NEGATIVE))
		{
			...
			hprintf255(request, "<font size=1>Last sync at minute: %dmin. Boosting from %dh to %dh<br> Relay Thresholds: On: %d Wh, Off: %dWh<br> Power: %d, Energy: %d <br></font>", 
				lastsync, bypass_on_time, bypass_off_time, dump_load_on, dump_load_off, (int)sensors[OBK_POWER].lastReading, (int)energyWh);
		}		
		/********************************************************************************************************************/
