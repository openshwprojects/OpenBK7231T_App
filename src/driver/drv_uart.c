...
int UART_InitUARTEx(int auartindex, int baud, int parity,int stop_bits,int data_width, bool hwflowc)
{
  uartbuf_t* fuartbuf = UART_GetBufFromPort(auartindex);
  fuartbuf->g_uart_init_counter++;
#ifdef UART_2_UARTS_CONCURRENT
    HAL_UART_InitEx(auartindex, baud, parity,stop_bits,data_width, hwflowc, -1, -1);
#else
...
int UART_InitUART(int baud, int parity,int stop_bits,int data_width, bool hwflowc) {
  int fuartindex = UART_GetSelectedPortIndex();
  return UART_InitUARTEx(fuartindex, baud, parity,stop_bits,data_width, hwflowc);
}
...
commandResult_t CMD_UART_Init(const void *context, const char *cmd, const char *args, int cmdFlags) {
    ...
    UART_InitUARTEx(fuartindex, baud, 0,0,3, false);
