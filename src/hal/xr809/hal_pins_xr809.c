...
// GPIO functionality
int HAL_PIN_CanThisPinBePWM(int index) { return 0; }
...
void HAL_PIN_SetOutputValue(int index, int iVal) {
	int port, pin;
	PIN_XR809_GetPortPinForIndex(index, &port, &pin);
	HAL_GPIO_WritePin(port, pin, iVal);
}

int HAL_PIN_ReadDigitalInput(int index) {
	int port, pin;
	PIN_XR809_GetPortPinForIndex(index, &port, &pin);
	return HAL_GPIO_ReadPin(port, pin) == GPIO_PIN_LOW ? 0 : 1;
}
