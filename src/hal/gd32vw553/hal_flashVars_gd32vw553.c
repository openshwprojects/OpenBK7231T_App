...
static int ReadFlashVars(void* target, int dataLen)
{
	...
	readLen = nvds_data_get(NULL, "OBK", KV_KEY_FLASH_VARS, target, &readLen);
...
static int SaveFlashVars(void* src, int dataLen)
{
	...
	res = nvds_data_put(NULL, "OBK", KV_KEY_FLASH_VARS, src, dataLen);
	if(res == NVDS_E_FAIL)
	{
		ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: failed for %d b", __func__, dataLen);
		return 0;
	}
	if(res == NVDS_E_INVAL_PARAM)
	{
		ADDLOG_DEBUG(LOG_FEATURE_CFG, "%s: param err for %d b", __func__, dataLen);
		return 0;
	}
...
void HAL_FlashVars_SaveEnergy(ENERGY_DATA** data, int channel_count)
{
	if(data != NULL)
	{
		for(int i = 0; i < channel_count; i++)
		{
			...
			nvds_data_put(NULL, "OBK", buffer, (uint8_t*)data[i], sizeof(ENERGY_DATA));
		}
	}
}
void HAL_FlashVars_GetEnergy(ENERGY_DATA* data, ENERGY_CHANNEL channel)
{
	if(data != NULL)
	{
		...
		nvds_data_get(NULL, "OBK", buffer, (uint8_t*)data, &len);
	}
}
//#endif
