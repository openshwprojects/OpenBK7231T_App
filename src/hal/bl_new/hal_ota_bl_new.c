...
int http_rest_post_flash(http_request_t* request, int startaddr, int maxaddr)
{
	...
	__attribute__((aligned(32))) bflb_mtd_handle_t handle;
	//init_ota(startaddr);
...
#define OTA_PROGRAM_SIZE (512)
	...
	ret = bflb_mtd_open(BFLB_MTD_PARTITION_NAME_FW_DEFAULT, &handle, BFLB_MTD_OPEN_FLAG_BACKUP);
	if (ret)
	{
	...
	uint8_t activeID;
	bflb_partition_config_t ptEntry;

	activeID = bflb_boot2_get_active_partition();

	printf("Starting OTA. OTA bin addr is %p, incoming len %i\r\n", recv_buffer, writelen);

	if (bflb_boot2_get_active_entries(BOOT2_PARTITION_TYPE_FW, &ptEntry))
	{
	...
	while (erase_offset < bin_size)
	{
		...
		bflb_mtd_erase(handle, erase_offset, erase_len);
	...
	do
	{
		...
		if (ota_header && useLen)
		{
			...
			bflb_mtd_write(handle, flash_offset, useLen, (byte*)useBuf);
			flash_offset += useLen;
		}
	...
	bflb_boot2_update_ptable(&ptEntry);
...
int HAL_FlashRead(char*buffer, int readlen, int startaddr) {
	int res;
	res = bflb_flash_read(startaddr, (uint8_t *)buffer, readlen);
	return res;
}
