...
void wl_status(void* ctxt)
{
	...
	case RW_EVT_STA_GOT_IP:
		...
		if(!g_cfg.use_adv)
		{
			...
			g_cfg.channel = status.channel;
			os_strcpy((char*)g_cfg.bssid, (char*)status.bssid);
			g_cfg.security = status.security;
			...
			memcpy(g_cfg.psk, psks, sizeof(g_cfg.psk));
			g_cfg.use_adv = true;
...
void HAL_ConnectToWiFi(const char* oob_ssid, const char* connect_key, obkStaticIP_t* ip)
{
	...
	if(g_cfg.use_adv)
	{
		...
		network_cfg.key_len = os_strlen(g_cfg.psk);
		memcpy(network_cfg.key, g_cfg.psk, network_cfg.key_len);
		os_strcpy(network_cfg.ap_info.bssid, g_cfg.bssid);
		...
		network_cfg.ap_info.channel = g_cfg.channel;
		network_cfg.ap_info.security = g_cfg.security;
